                    ...
                    for add_encoding in "${add_encodings_hg[@]}"; do
                        if [ "$add_encoding" == "True" ]; then
                            for encoding in "${encodings[@]}"; do
                                if [ "$encoding" == "RW" ]; then
                                    for rw_type in "${rw_types[@]}"; do
                                        combinations+=("$model $dataset $encoding $rw_type $transformer none none $nlayer $add_encoding")
                                    done
                                elif [ "$encoding" == "LCP" ]; then
                                    for curvature_type in "${curvature_types[@]}"; do
                                        combinations+=("$model $dataset $encoding $curvature_type $transformer none none $nlayer $add_encoding")
                                    done
                                elif [ "$encoding" == "Laplacian" ]; then
                                    for laplacian_type in "${laplacian_types[@]}"; do
                                        combinations+=("$model $dataset $encoding $laplacian_type $transformer none none $nlayer $add_encoding")
                                    done
                                else
                                    combinations+=("$model $dataset $encoding none $transformer none none $nlayer $add_encoding")
                                fi
                            done
                        else
                            combinations+=("$model $dataset none none $transformer none none $nlayer $add_encoding")
                        fi
                    done
                done
            done
