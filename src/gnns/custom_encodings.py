...
class EdgeCurvature(BaseTransform):
    ...
    def compute_orc(self, data: Data) -> Data:
        ...
        edge_curv = torch.tensor(
            [
                orc.G.edges[edge]["ricciCurvature"]["rc_curvature"]
...
class LocalCurvatureProfile(BaseTransform):
        ...
        for node in graph.nodes():
            if len(neighbors[node]) > 0:
                min_orc.append(
                    min(
                        [
                            orc.G.edges[node, neighbor]["ricciCurvature"][
                                "rc_curvature"
                            ]
    ...
    def forward(
        self, data: Data
    ) -> Data:  # renamed this forward from compute_orc_approx
        graph = to_networkx(data)
        ...
        def compute_upper_bound(node_1, node_2):
            ...
            num_triangles = len(
                [
                    neighbor
                    for neighbor in neighbors[node_1]
                    if neighbor in neighbors[node_2]
                ]
            )
            ...
            return (
                -np.max(
                    [
                        ...
                        - 1 / deg_node_1
                        - 1 / deg_node_2
                        - num_triangles / np.min([deg_node_1, deg_node_2]),
                    ]
                )
                - np.max(
                    [
                        ...
                        - 1 / deg_node_1
                        - 1 / deg_node_2
                        - num_triangles / np.max([deg_node_1, deg_node_2]),
                    ]
                )
                + num_triangles / np.max([deg_node_1, deg_node_2])
            )
        ...
        for node in graph.nodes():
            if len(neighbors[node]) > 0:
                min_orc.append(
                    min(
                        [
                            compute_lower_bound(node, neighbor)
                            for neighbor in neighbors[node]
                        ]
                    )
                )
                max_orc.append(
                    max(
                        [
                            compute_upper_bound(node, neighbor)
                            for neighbor in neighbors[node]
                        ]
                    )
                )
    ...
    def compute_afrc_3(self, data: Data) -> Data:
        ...
        # compute the min, max, mean, std, and median of the ORC for each node
        min_afrc_3 = [
            min([afrc_3.G[node][neighbor]["AFRC"] for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
        max_afrc_3 = [
            max([afrc_3.G[node][neighbor]["AFRC"] for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
        mean_afrc_3 = [
            np.mean([afrc_3.G[node][neighbor]["AFRC"] for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
        std_afrc_3 = [
            np.std([afrc_3.G[node][neighbor]["AFRC"] for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
        median_afrc_3 = [
            np.median(
                [afrc_3.G[node][neighbor]["AFRC"] for neighbor in neighbors[node]]
            )
            for node in graph.nodes()
        ]
        ...
        # create a torch.tensor of dimensions (num_nodes, 5) containing the min, max, mean, std, and median of the ORC for each node
        lcp_pe = torch.tensor(
            [min_afrc_3, max_afrc_3, mean_afrc_3, std_afrc_3, median_afrc_3]
        ).T
    ...
    def compute_afrc_4(self, data: Data) -> Data:
        ...
        # compute the min, max, mean, std, and median of the ORC for each node
        min_afrc_4 = [
            min([afrc_4.G[node][neighbor]["AFRC_4"] for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
        max_afrc_4 = [
            max([afrc_4.G[node][neighbor]["AFRC_4"] for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
        mean_afrc_4 = [
            np.mean(
                [afrc_4.G[node][neighbor]["AFRC_4"] for neighbor in neighbors[node]]
            )
            for node in graph.nodes()
        ]
        std_afrc_4 = [
            np.std([afrc_4.G[node][neighbor]["AFRC_4"] for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
        median_afrc_4 = [
            np.median(
        ...
        # create a torch.tensor of dimensions (num_nodes, 5) containing the min, max, mean, std, and median of the ORC for each node
        lcp_pe = torch.tensor(
            [min_afrc_4, max_afrc_4, mean_afrc_4, std_afrc_4, median_afrc_4]
        ).T
    ...
    def compute_frc(self, data: Data) -> Data:
        ...
        min_frc = [
            min([frc(node, neighbor) for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
        max_frc = [
            max([frc(node, neighbor) for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
        mean_frc = [
            np.mean([frc(node, neighbor) for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
        std_frc = [
            np.std([frc(node, neighbor) for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
        median_frc = [
            np.median([frc(node, neighbor) for neighbor in neighbors[node]])
            for node in graph.nodes()
        ]
    ...
    def compute_orc(self, data: Data) -> Data:
        ...
        for node in graph.nodes():
            if len(ordered_orc[node]) > 5:
                ...
                cropped_orc.append(
                    ordered_orc[node] + [0] * (5 - len(ordered_orc[node]))
                )
            else:
