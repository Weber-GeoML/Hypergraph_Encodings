...
for i in range(len(train_datasets)):
    ...
    print(
        f"TESTING: {train_name} {test_name} ({args.rewiring} - layer {args.layer_type})"
    )

    # encode the train and test datasets using the given encoding, if args.encoding is not None
    for dataset_type, pair in zip(
        ["train", "test"], [train_datasets[i], test_datasets[i]]
    ):
        name, dataset = pair

        if hasattr(args, "encoding") and args.encoding in [
            "LAPE",
            "RWPE",
            "LCP",
            "LDP",
    ...
    log_to_file(
        f"RESULTS FOR {train_name} {test_name} ({args.rewiring}), {args.num_iterations} ITERATIONS:\n"
    )
    ...
    with open(
        f"results/graph_regression_{args.layer_type}_{args.rewiring}.csv", "a"
    ) as f:
