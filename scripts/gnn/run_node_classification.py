...
if args.encoding in ["LAPE", "RWPE", "LCP", "LDP", "SUB", "EGO"]:
    if args.encoding == "LAPE":
        ...
        print("Encoding Local Curvature Profile (ORC)")
...
for key in datasets:
    ...
    if args.rewiring == "fosr":
        edge_index, edge_type, _ = fosr.edge_rewire(
            dataset.data.edge_index.numpy(), num_iterations=args.num_iterations
        )
        ...
        curvature_type = "bfc"
        dataset.data.edge_index, dataset.data.edge_type = sdrf.sdrf(
            dataset.data,
        ...
        print(
            f"[INFO] BORF hyper-parameter : num_iterations = {args.borf_batch_remove}"
        )
        dataset.data.edge_index, dataset.data.edge_type = borf.borf3(
            dataset.data,
            loops=args.num_iterations,
            remove_edges=False,
            is_undirected=True,
        ...
        for i in range(len(dataset)):
            dataset[i].edge_index, dataset[i].edge_type = borf.borf4(
                dataset[i],
                loops=args.num_iterations,
                remove_edges=False,
                is_undirected=True,
        ...
        for i in range(len(dataset)):
            dataset[i].edge_index, dataset[i].edge_type = dropout_edge(
                dataset[i].edge_index, dataset[i].edge_type, p=p, force_undirected=True
            )
    ...
    for trial in range(args.num_trials):
        ...
        for i in range(args.num_splits):
            train_acc, validation_acc, test_acc = Experiment(
