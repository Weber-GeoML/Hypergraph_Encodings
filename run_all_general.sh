...
#SBATCH --output=sbatch_logs/ablation_%j.log  # Standard output and error log (with job ID)
                    ...
                    for dataset in "${datasets[@]}"; do
                        for add_encoding in "${add_encodings[@]}"; do
                            if [ "$add_encoding" == "True" ]; then
                                for encoding in "${encodings[@]}"; do
                                    if [ "$encoding" == "RW" ]; then
                                        for rw_type in "${rw_types[@]}"; do
                                            ...
                                            python scripts/train_val.py --add-self-loop --model="$model" --data="$data_type" --dataset="$dataset" --encodings="$encoding" --random-walk-type="$rw_type" $([ "$transformer" == "True" ] && echo "--do-transformer") $([ "$transformer" == "False" ] && echo "--no-transformer") $([ "$transformer" == "True" ] && echo "--transformer-version=$transformer_version") $([ "$transformer" == "True" ] && echo "--transformer-depth=$transformer_depth") > "$log_file" 2>&1
                                        done
                                        ...
                                        for curvature_type in "${curvature_types[@]}"; do
                                            ...
                                            python scripts/train_val.py --add-self-loop --model="$model" --data="$data_type" --dataset="$dataset" --encodings="$encoding" --curvature-type="$curvature_type" $([ "$transformer" == "True" ] && echo "--do-transformer") $([ "$transformer" == "False" ] && echo "--no-transformer") $([ "$transformer" == "True" ] && echo "--transformer-version=$transformer_version") $([ "$transformer" == "True" ] && echo "--transformer-depth=$transformer_depth") > "$log_file" 2>&1
                                        done
                                        ...
                                        for laplacian_type in "${laplacian_types[@]}"; do
                                            ...
                                            python scripts/train_val.py --add-self-loop --model="$model" --data="$data_type" --dataset="$dataset" --encodings="$encoding" --laplacian-type="$laplacian_type" $([ "$transformer" == "True" ] && echo "--do-transformer") $([ "$transformer" == "False" ] && echo "--no-transformer") $([ "$transformer" == "True" ] && echo "--transformer-version=$transformer_version") $([ "$transformer" == "True" ] && echo "--transformer-depth=$transformer_depth") > "$log_file" 2>&1
                                        done
                                    else
                                        ...
                                        python scripts/train_val.py --add-self-loop --model="$model" --data="$data_type" --dataset="$dataset" --encodings="$encoding" $([ "$transformer" == "True" ] && echo "--do-transformer") $([ "$transformer" == "False" ] && echo "--no-transformer") $([ "$transformer" == "True" ] && echo "--transformer-version=$transformer_version") $([ "$transformer" == "True" ] && echo "--transformer-depth=$transformer_depth") > "$log_file" 2>&1
                                    fi
                                done
                            else
                                ...
                                python scripts/train_val.py --add-self-loop --model="$model" --data="$data_type" --dataset="$dataset" --add-encodings=False $([ "$transformer" == "True" ] && echo "--do-transformer") $([ "$transformer" == "False" ] && echo "--no-transformer") $([ "$transformer" == "True" ] && echo "--transformer-version=$transformer_version") $([ "$transformer" == "True" ] && echo "--transformer-depth=$transformer_depth") > "$log_file" 2>&1
                            fi
                        done
                    done
                done
