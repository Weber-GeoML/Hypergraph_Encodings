...
def test_ldp_encoding(sample_hypergraph: dict[str, dict[str, Any] | int | np.ndarray], encoder: HypergraphEncodings) -> None:
    """Test LDP encoding."""
...
def test_rwpe_encoding(sample_hypergraph: dict[str, dict[str, Any] | int | np.ndarray], encoder: HypergraphEncodings) -> None:
    """Test RWPE encoding."""
    k = 2
...
def test_lcp_orc_encoding(sample_hypergraph: dict[str, dict[str, Any] | int | np.ndarray], encoder: HypergraphEncodings) -> None:
...
def test_lape_normalized(sample_hypergraph: dict[str, dict[str, Any] | int | np.ndarray], encoder: HypergraphEncodings) -> None:
    ...
    assert "features" in result
    assert isinstance(result["features"], np.ndarray)
    assert result["features"].shape[1] == k + 1  # k + 1 features per node (includes original feature)

def test_invalid_encoding(sample_hypergraph: dict[str, dict[str, Any] | int | np.ndarray], encoder: HypergraphEncodings) -> None:
    """Test invalid encoding name."""
    result = get_encodings(sample_hypergraph, encoder, "INVALID")
    assert result is None

def test_copy_not_modified(sample_hypergraph: dict[str, dict[str, Any] | int | np.ndarray], encoder: HypergraphEncodings) -> None:
