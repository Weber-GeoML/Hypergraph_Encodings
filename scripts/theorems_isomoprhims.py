...
def test_laplacian(hg1, hg2, lap_type, name1="Graph1", name2="Graph2"):
    ...
    if True:
        ...
        print(f"Maximum difference: {np.max(np.abs(hg1_lape['features'] - hg2_lape['features']))}")

    # Save the heatmap of the difference of the features
    plt.imshow(np.abs(hg1_lape['features'] - hg2_lape['features']), cmap="hot", interpolation="nearest")
    plt.colorbar()
    ...
    if False:
        ...
        assert np.allclose(L1, L2, rtol=1e-12, atol=1e-12), "The two laplacians are not the same"
    ...
    assert hg1_lape["features"].shape == hg2_lape["features"].shape, "The two LAPE encodings have different shapes"
    print("*"*100)
...
def check_permutation_equivalence(matrix1, matrix2, tolerance=1e-15):
    ...
    if matrix1.shape != matrix2.shape:
        print(f"The two matrices have different shapes: {matrix1.shape} and {matrix2.shape}")
        return False, None
...
def compute_and_compare_encodings(G1, G2, name1="Graph1", name2="Graph2"):
    ...
    assert np.all(hg1_ldp["features"] == hg2_ldp["features"]), "The two degree distributions are not the same"
    ...
    encoder_shrikhande = HypergraphEncodings()
    encoder_rooke = HypergraphEncodings()
    hg1_rw = encoder_shrikhande.add_randowm_walks_encodings(hg1.copy(), rw_type="WE", verbose=False)
    hg2_rw = encoder_rooke.add_randowm_walks_encodings(hg2.copy(), rw_type="WE", verbose=False)
    print(f"Maximum difference: {np.max(np.abs(hg1_rw['features'] - hg2_rw['features']))}")

    # assert that the two random walk distributions are the same
    assert np.allclose(hg1_rw["features"], hg2_rw["features"], rtol=1e-12, atol=1e-12), "The two random walk distributions differ beyond 1e-12 tolerance"
